...
struct CPUTrainingView: View {
    ...
    func calcWidth(fighter: Fighter) -> CGFloat {
        DispatchQueue.main.async {
            ...
            if fightLogic.getFighter(player: 0).name == previousFighter {
    ...
    var body: some View {
                    ...
                    HStack(spacing: 10) {
                        Group {
                            ZStack {
                                ...
                                CustomText(key: "waiting on other player", fontSize: 14).frame(width: geometry.size.height - 60, height: 85, alignment: .topLeading).padding(.horizontal, 15)
                            }
        ...
        .onAppear {
            previousFighter = fightLogic.getFighter(player: 0).name
