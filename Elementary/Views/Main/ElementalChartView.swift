                    ...
                    ForEach(element.weaknesses.indices, id: \.self) { index in
                        if let temp = GlobalData.shared.elements[element.weaknesses[index]] {
                            ZStack {
                                ...
                                Text(createSymbol(symbol: temp.symbol)).font(.custom("Font Awesome 5 Pro", size: smallFont)).foregroundColor(Color.white)
                            }
                        }
                    }
                    Text(createSymbol(symbol:"0xf061")).frame(maxWidth: .infinity).font(.custom("Font Awesome 5 Pro", size: smallFont)).foregroundColor(Color.white)
                    ZStack {
                        Rectangle().fill(Color(hex: element.color))
                        Text(createSymbol(symbol: element.symbol)).font(.custom("Font Awesome 5 Pro", size: smallFont)).foregroundColor(Color.white)
                    }
                    Text(createSymbol(symbol:"0xf061")).frame(maxWidth: .infinity).font(.custom("Font Awesome 5 Pro", size: smallFont)).foregroundColor(Color.white)
